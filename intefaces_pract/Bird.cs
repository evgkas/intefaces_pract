...
namespace intefaces_pract
{
    public class Bird : IFlyable
    {
        ...
        {
            double distance = Math.Sqrt(Math.Pow((x - x0), 2) + Math.Pow((y - y0), 2)); //difference in height not calculated
            if ( distance > 50 || z > 5)
            {
        ...
        public double GetFlyTime(double x, double y) //flying on the same height
        {
